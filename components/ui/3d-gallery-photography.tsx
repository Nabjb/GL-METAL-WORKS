...
interface InfiniteGalleryProps {
	...
	damping?: number;
	autoPlayForce?: number;
	externalScrollVelocity?: number;
	className?: string;
...
const MAX_HORIZONTAL_OFFSET = 18;
const MAX_VERTICAL_OFFSET = 14;
...
function GalleryScene({
  ...
  const materials = useMemo(
	...
	const spatialPositions = useMemo(() => {
		...
		for (let i = 0; i < visibleCount; i++) {
			// Distribute planes around the camera with wider angular coverage.
			const angle = (i * 2.399963229728653) % (Math.PI * 2); // Golden angle
			const ring = Math.floor(i / 5);
			const radial = 0.5 + ring * 0.4;
			const jitterX = (Math.sin(i * 1.7) * 0.25);
			const jitterY = (Math.cos(i * 1.3) * 0.2);

			const x = Math.cos(angle) * radial * maxHorizontalOffset + jitterX;
			const y = Math.sin(angle) * radial * maxVerticalOffset + jitterY;
  ...
  // Handle scroll input
	const handleWheel = useCallback(
		(event: WheelEvent) => {
			...
			setScrollVelocity((prev) => prev + event.deltaY * scrollMultiplier * speed);
			setAutoPlay(false);
  ...
  useEffect(() => {
	...
	useFrame((state, delta) => {
		if (externalScrollVelocity !== 0) {
			setScrollVelocity((prev) => prev + externalScrollVelocity * speed);
			setAutoPlay(false);
			lastInteraction.current = Date.now();
		}
		...
		// Apply auto-play
		if (autoPlay) {
			setScrollVelocity((prev) => prev + autoPlayForce * delta);
		}
...
export default function InfiniteGallery({
  ...
  if (!webglSupported) {
	...
	return (
		<div className={className} style={style}>
			<Canvas camera={{ position: [0, 0, 0], fov: 65 }} gl={{ antialias: true, alpha: true }}>
				<GalleryScene
					images={images}
					fadeSettings={fadeSettings}
					blurSettings={blurSettings}
					...
					damping={damping}
					autoPlayForce={autoPlayForce}
					externalScrollVelocity={externalScrollVelocity}
				/>
			</Canvas>
		</div>
	);
